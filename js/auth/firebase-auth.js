...
export async function initFirebaseAuth({debugLog,onStatus,onAuthStatus,onUserChanged,onReady}){
  ...
  try {
    const [{ initializeApp }, { getFirestore, doc, setDoc, collection, getDocs, query, orderBy, limit }, { getAuth, GoogleAuthProvider, signInWithPopup, signInWithRedirect, signInAnonymously, getRedirectResult, onAuthStateChanged, signOut }] = await Promise.all([
    ...
    const FIRESTORE = { db, doc, setDoc, collection, getDocs, query, orderBy, limit };
